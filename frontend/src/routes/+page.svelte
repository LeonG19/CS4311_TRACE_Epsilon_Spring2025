<script>
  ...
  async function handleStart() {
      try{
          const response = await fetch(`http://169.254.7.176:8000/analyst/${initials}/`,{
              method: 'POST'
          });
  ...
  async function handleInitCreation(type) {
    try {
      const verification = await fetch(`http://169.254.7.176:8000/analyst/${initials}/`,{
         method: 'POST'
      });
      ...
      const response = await fetch(`http://169.254.7.176:8000/create_initials/${initials}/${type}/${name}`,{
          method: 'POST',
      });
