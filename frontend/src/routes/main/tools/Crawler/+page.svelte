<script>
  ...
  function startPolling() {
    ...
    pollingInterval = setInterval(async () => {
      ...
      const response = await fetch(`http://169.254.7.176:8000/crawler_status?job_id=${jobId}`);
      if (response.ok) {
        ...
        if (status.status === 'finished') {
          stopPolling();
          try{
            const response = await fetch(`http://169.254.7.176:8000/submit_results/crawler/${sessionStorage.getItem('name')}`, {
              method: 'POST',
              headers: {
  ...
  async function handleSubmit() {
    ...
    try {
      const validateURL = await fetch('http://169.254.7.176:8000/validate_url', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    try {
      const response = await fetch('http://169.254.7.176:8000/crawler', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async function stopCrawler() {
    ...
    if (!jobId || jobId === 'null') return;
    const response = await fetch(`http://169.254.7.176:8000/stop_crawler?job_id=${jobId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (response.ok) {
      if (jobId && jobId !== 'null') {
        let statusResp = await fetch(`http://169.254.7.176:8000/crawler_status?job_id=${jobId}`);
        if (statusResp.ok) {
  ...
  async function pauseCrawler(){
    ...
    stopPolling();
    if (!jobId || jobId === 'null') return;
    const response = await fetch(`http://169.254.7.176:8000/pause_crawler?job_id=${jobId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async function resumeCrawler(){
    ...
    startTimer();
    if (!jobId || jobId === 'null') return;
    const response = await fetch(`http://169.254.7.176:8000/resume_crawler?job_id=${jobId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
