<script>
    ...
    let dbInputs = [
      ...
      },
      {
        ...
        value: "8000",
        example: "8000",
        tooltip: "Default MySQL port is 8000. Adjust if needed.",
        required: true
      },
      {
        id: "username",
    ...
    async function handleSubmit() {
      ...
      try {
        const response = await fetch('http://localhost:8000/api/db_enum', {
          method: 'POST',
      ...
      {#each dbInputs as input}
        <label title={input.tooltip}>
          <span>
            {input.label}
            {#if input.id === "host"}
              <i class="fas fa-server" style="margin-left: 5px;"></i>
            {/if}
          </span>
          <input
            type={input.type}
            ...
            oninput={(e) => dynamicParamUpdate(input.id, e.target.value)}
          />
