<script>
  ...
  async function handleRowClick(run_id){
    const response = await fetch(`http://169.254.7.176/getScanResults/${run_id}`, {
      method: 'GET',
      headers: {
  ...
  onMount(async () => {
    ...
    try{
      const response = await fetch(`http://169.254.7.176/getScan/${sessionStorage.getItem('name')}/${typeOfTool}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
