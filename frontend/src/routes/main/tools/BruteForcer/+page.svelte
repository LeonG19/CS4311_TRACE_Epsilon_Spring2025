<script>
  ...
  async function apiCall(endpoint, onSuccess, messagePrefix) {
    try {
      const response = await fetch(`http://169.254.7.176:8000/${endpoint}`, {
        method: 'POST',
  ...
  async function handleFile(event) {
    ...
    try {
      const response = await fetch('http://169.254.7.176:8000/upload-wordlist', {
        method: 'POST',
        body: formData
      });
  ...
  async function handleSubmit() {
    ...
    try {
      const response = await fetch('http://169.254.7.176:8000/bruteforcer', {
        method: 'POST',
        headers: {
...
async function submitbruteResultsToProject() {
    try {
      const resp = await fetch(
        `http://169.254.7.176:8000/submit_results/bruteforcer/${projectName}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
