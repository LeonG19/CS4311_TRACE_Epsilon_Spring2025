<script>
  ...
  async function exportFromDB(file){
    ...
    try {
      const response = await fetch(("http://169.254.7.176:8000/export_AI/" + projectName + "/" + file), {
        method: "GET"
      });
  ...
  async function handleDelete(file){
    console.log(file)
    try {
      const response = await fetch(("http://169.254.7.176:8000/delete_AI/"+file), {
        method: "GET"
      });
  ...
  try {
    const response = await fetch("http://169.254.7.176:8000/stop_AI", {
      method: "POST"
    });
  ...
  async function fetchUserList() {
    try {
      const response = await fetch(("http://169.254.7.176:8000/ai_results/" + projectName), {
        method: "GET",
      });
      const data = await response.json();
  ...
  async function handleSubmit() {
    ...
    try{
      const response = await fetch('http://169.254.7.176:8000/generate-credentials', {
        method: 'POST',
        body: formData
      });
  ...
  async function handleSave() {
    ...
    try{
      const response = await fetch(("http://169.254.7.176:8000/submit_txt_results/AI/"+projectName), {
      method: "POST",
      body: formData
      });
