<script>
  ...
  async function handleSubmit(event) {
    ...
    try {
      progress = 50;
      const response = await fetch('http://169.254.7.176:5173/api/db_enumerator', {
        method: 'POST',
  ...
  async function submitToBackend() {
    ...
    try {
      const response = await fetch(`http://169.254.7.176:5173/submit_results/db_enum/${projectName}`, {
        method: "POST",
        headers: {
