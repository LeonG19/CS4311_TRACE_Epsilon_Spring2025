<script>
  ...
  async function restoreProject(projectName) {
    try {
      const response = await fetch(`http://169.254.7.176:5173/restore/${projectName}/`, {
        method: 'POST'
      });
      if (response.ok) {
  ...
  async function sendTestData() {
    try {
      ...
      const response = await fetch('http://169.254.7.176:5173/submit_txt_results/AI/Hacking_Mexico', {
      method: 'POST',
