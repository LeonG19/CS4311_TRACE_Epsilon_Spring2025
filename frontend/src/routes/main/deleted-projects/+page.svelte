<script>
  ...
  async function restoreProject(projectName) {
    try {
      const response = await fetch(`http://${import.meta.env.VITE_API_URL}/restore/${projectName}/`, {
        method: 'POST'
      });
      if (response.ok) {
  ...
  async function sendTestData() {
    try {
      ...
      const response = await fetch('http://${import.meta.env.VITE_API_URL}/submit_txt_results/AI/Hacking_Mexico', {
      method: 'POST',
