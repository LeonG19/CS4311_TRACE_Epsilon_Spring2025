<script>
    ...
    async function fetchProjects() {
      try {
        const response = await fetch(`http://${import.meta.env.VITE_API_URL}/dashboard/${initials}`);
        if (!response.ok) {
    ...
    try {
      const response = await fetch(`http://${import.meta.env.VITE_API_URL}/lock/${projectName}/${analyst_initials}`, {
        method: 'POST'
      });
      if (response.ok) {
        // Update myProjects
  ...
  async function restoreProject(projectName, analyst_initials) {
    try {
      const response = await fetch(`http://${import.meta.env.VITE_API_URL}/unlock/${projectName}/${analyst_initials}`, {
        method: 'POST'
      });
      if (response.ok) {
        // Update myProjects
  ...
  async function deleteProject(projectName) {
        try{
            const response= await fetch(`http://${import.meta.env.VITE_API_URL}/delete/${projectName}`,{
                method: 'POST'
            });
            if (response.ok){
                fetchProjects();
    ...
    try {
      const response = await fetch(`http://${import.meta.env.VITE_API_URL}/export/${projectName}`, {
        method: 'GET'
      });
      if (!response.ok) {
