<script>
    ...
    try {
      const response = await fetch(`http://169.254.7.176/lock/${projectName}/${analyst_initials}`, {
        method: 'POST'
      });
      if (response.ok) {
        // Update myProjects
  ...
  async function restoreProject(projectName, analyst_initials) {
    try {
      const response = await fetch(`http://169.254.7.176/unlock/${projectName}/${analyst_initials}`, {
        method: 'POST'
      });
      if (response.ok) {
        // Update myProjects
  ...
  async function deleteProject(projectName) {
    ...
    try {
      const response = await fetch(`http://169.254.7.176/export/${projectName}`, {
        method: 'GET'
      });
      if (!response.ok) {
